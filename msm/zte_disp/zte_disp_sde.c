...
#include "zte_disp_sde.h"
#include "../sde/sde_encoder.h"

#define to_sde_encoder_virt(x) container_of(x, struct sde_encoder_virt, base)

static struct calc_hw_vsync g_calc_hw_vsync[2];
...
static int zte_disp_vsync_show(struct seq_file *m, void *v)
{
    struct calc_hw_vsync *calc_vsync;
    ...
    pr_info("[MSM_LCD]fps: %d.%d, vsync_period_ns:%lld,"
	...
	seq_printf(m, "fps: %d.%d  vsync_period_ns:%lld"
...
static int zte_disp_vsync_open(struct inode *inode, struct file *file)
{
	return single_open(file, zte_disp_vsync_show, NULL);
}

static const struct proc_ops zte_disp_vsync_proc_fops = {
	.proc_open	= zte_disp_vsync_open,
  	.proc_read	= seq_read,
  	.proc_lseek	= seq_lseek,
 	.proc_release	= single_release,
};

int zte_disp_vsync_create(void)
{
	proc_create("driver/disp_vsync", 0664, NULL, &zte_disp_vsync_proc_fops);
	pr_info("MSM_LCD create vysnc node for panel debug");
	return 0;
}
